...
export class MarshalsService {
    ...
    async findOne(id: string): Promise<Marshal> {
        // @ts-ignore
        return await this.marshalRep.findBy({id}, {
            relations: ['battles']
        });
    }

    async create(objDto: AddMarshalDto): Promise<Marshal> {
        const battles: Array<Battle> = await Promise.all(
            objDto.battles.map((name: string) => this.preloadBattleByName(name))
        )
    ...
    async update(id: string, objDto: UpdateMarshalDto): Promise<Marshal> {
        const battles: Array<Battle> =objDto.battles && (await Promise.all(
            objDto.battles.map((name: string) => this.preloadBattleByName(name))
        ))
